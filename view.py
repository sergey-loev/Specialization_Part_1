...
def main_menu() -> int:
    ...
    while True:
        choice = input(messages.input_choice)
...
def input_notes(new_id: int, message: str, cancel: str) -> dict[str, str]:
    ...
    for key, value in messages.new_notes.items():
        data = input(value)
        match key:
            case 'title':
                if data:
...
def input_index(message: str, notes: list, error: str):
    print_notes(notes, error)
    while True:
        if(len(notes) == 0):
            return -1
        index = input(message)
        if index.isdigit() and 0 < int(index) <= len(notes):
            return int(index)
...
def input_date(message):
    while True:
        ...
        try:
            ...
            print("Некорректный формат, необходимо ввести дату в формате DD.MM.YYYY")
